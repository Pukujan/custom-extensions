...
No further live action is required for the accepted v0.1 pilot/holdout baseline. Run official-import validation only if the user supplies a ZIP; run account-wide or streaming/SSE validation only after a separate explicit checkpoint. Do not start account-wide export automatically.
