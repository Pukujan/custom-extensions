...
Current P0 task: `TASK-PROV-0008-controlled-validation.md` (built-in full-CDP pilot and required unpacked-MV3 smoke accepted; account-wide live export is authorized but blocked by the current browser surface).
...
Next atomic action: run the authorized account-wide exporter in a connected Chromium/Brave MV3 surface, then validate the controlled account manifest/catalog/per-conversation reports and SHA-256 index. Run official-import validation only if the user supplies a ZIP. Do not claim account-wide acceptance from the current in-app-browser probes.
