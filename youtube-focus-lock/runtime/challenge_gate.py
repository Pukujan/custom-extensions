#!/usr/bin/env python3
"""Cross-platform challenge engine for YouTube Focus Lock.

Candidate solutions always execute in a separate normal-user Python process. The
process timeout is portable; POSIX hosts additionally apply CPU/address-space
resource limits. Privileged policy changes never execute candidate code as an
...
def default_install_dir() -> Path:
    override = os.environ.get("YFL_INSTALL_DIR")
    if override:
        return Path(override)
    if os.name == "nt":
        return Path(os.environ.get("PROGRAMDATA", r"C:\ProgramData")) / "YouTubeFocusLock"
    return Path("/Library/Application Support/YouTubeFocusLock")
...
PROOF_SECRET_PATH = STATE_DIR / "proof-secret"


def configure_state_dir(path: Path) -> None:
    global STATE_DIR, CHALLENGE_ROOT, TOKEN_PATH, PROOF_SECRET_PATH
...
def write_challenge(mode="preview", now=None, rng=None) -> Path:
...
def runtime_hint(error_type, _message):
    mapping = {
...
def _apply_limits():
    if resource is not None:
        for limit_name, value in (
            ...
            try:
                resource.setrlimit(getattr(resource, limit_name), value)
...
def _load_candidate(path: Path, function_name: str):
...
def _worker_evaluate(problem_id, seed, solution_path, uid=None, gid=None):
    try:
        if uid is not None:
            if os.name != "posix":
                raise RuntimeError("UID/GID privilege drop is only available on POSIX")
            os.setgroups([])
            os.setgid(int(gid))
            os.setuid(int(uid))
        _apply_limits()
        ...
        return {"status": "runtime", "errorType": type(exc).__name__, "message": str(exc)[:400]}


def evaluate_problem(problem_id, seed, solution_path: Path, uid=None, gid=None, timeout_seconds=7):
    ...
    command = [
        ...
        "--problem-id", str(problem_id), "--seed", str(int(seed)),
    ...
    try:
        completed = subprocess.run(
            ...
            timeout=timeout_seconds, env=env, check=False, **kwargs
        )
    except subprocess.TimeoutExpired:
        return {
            "status": "timeout", "compile": comp,
    ...
    if state == "runtime":
        ...
        return {
            "status": "runtime", "compile": comp, "errorType": et,
            "message": payload.get("message", ""), "hint": runtime_hint(et, payload.get("message", "")),
        }
    if completed.returncode < 0 and os.name != "nt":
        sig = -completed.returncode
        return {
            "status": "timeout" if sig in (getattr(signal, "SIGALRM", -1), getattr(signal, "SIGKILL", -2)) else "runtime",
            "compile": comp, "message": "Candidate process ended by signal %d" % sig,
            "hint": "The solution was terminated. Check for infinite loops, runaway recursion, or excessive memory use.",
        }
    return {
        "status": "runtime", "compile": comp,
...
def _proof_payload(proof):
    keys = ("challengeId", "issuedAt", "expiresAt", "nonce", "codeHashes")
    return json.dumps({k: proof[k] for k in keys}, sort_keys=True, separators=(",", ":")).encode("utf-8")


def _proof_secret(create=False):
    if PROOF_SECRET_PATH.exists():
        return bytes.fromhex(PROOF_SECRET_PATH.read_text(encoding="utf-8").strip())
    if not create:
        raise FileNotFoundError(PROOF_SECRET_PATH)
    STATE_DIR.mkdir(parents=True, exist_ok=True)
    secret = secrets.token_bytes(32)
    PROOF_SECRET_PATH.write_text(secret.hex(), encoding="utf-8")
    try:
        os.chmod(PROOF_SECRET_PATH, 0o600)
    except OSError:
        pass
    return secret


def _code_hashes(root: Path, metadata):
    return [
        hashlib.sha256((root / item["dir"] / "solution.py").read_bytes()).hexdigest()
        for item in metadata["problems"]
    ]


def create_windows_proof(root: Path) -> Path:
    """Freshly re-run all five as the normal user, then bind proof to exact code."""
    metadata = json.loads((root / "challenge.json").read_text(encoding="utf-8"))
    now = int(time.time())
    if metadata.get("mode") != "locked":
        raise RuntimeError("Preview challenges cannot unlock maintenance.")
    if now >= int(metadata.get("expiresAt", 0)):
        raise RuntimeError("Challenge session expired.")
    for index, item in enumerate(metadata["problems"], 1):
        result = evaluate_problem(item["id"], int(item["seed"]), root / item["dir"] / "solution.py")
        if result.get("status") != "pass":
            raise RuntimeError("Final verification failed on problem %d (%s)." % (index, result.get("status")))
    proof = {
        "challengeId": metadata["id"], "issuedAt": now,
        "expiresAt": min(int(metadata["expiresAt"]), now + PROOF_SECONDS),
        "nonce": secrets.token_hex(16), "codeHashes": _code_hashes(root, metadata),
    }
    proof["signature"] = hmac.new(_proof_secret(create=True), _proof_payload(proof), hashlib.sha256).hexdigest()
    path = root / "maintenance-proof.json"
    write_json(path, proof)
    return path


def verify_windows_proof(root: Path) -> bool:
    try:
        metadata = json.loads((root / "challenge.json").read_text(encoding="utf-8"))
        proof = json.loads((root / "maintenance-proof.json").read_text(encoding="utf-8"))
        signature = str(proof.pop("signature"))
        expected = hmac.new(_proof_secret(create=False), _proof_payload(proof), hashlib.sha256).hexdigest()
        now = int(time.time())
        return (
            metadata.get("mode") == "locked"
            and proof.get("challengeId") == metadata.get("id")
            and hmac.compare_digest(signature, expected)
            and int(proof["issuedAt"]) <= now <= int(proof["expiresAt"])
            and int(proof["expiresAt"]) <= int(metadata["expiresAt"])
            and proof.get("codeHashes") == _code_hashes(root, metadata)
        )
    except (OSError, KeyError, ValueError, json.JSONDecodeError):
        return False


def _issue_token(challenge_id, username, target_dir: Path):
    if not SECRET_PATH.exists():
        raise RuntimeError("Maintenance secret missing: %s" % SECRET_PATH)
    now = int(time.time())
    token = {
        "challengeId": challenge_id, "issuedAt": now,
        "expiresAt": now + TOKEN_MINUTES * 60, "nonce": secrets.token_hex(16), "user": username,
    }
...
def unlock_windows(root: Path, username: str) -> int:
    if os.name != "nt" or not _windows_admin():
        print("Windows unlock must run from an elevated Administrator process.", file=sys.stderr)
        return 2
    try:
        metadata = json.loads((root / "challenge.json").read_text(encoding="utf-8"))
        ...
        return 2
    if not verify_windows_proof(root):
        print("Fresh normal-user five-problem verification proof is missing or invalid.", file=sys.stderr)
        return 1
    try:
        _issue_token(metadata["id"], username, STATE_DIR)
    except RuntimeError as exc:
        print(str(exc), file=sys.stderr)
        return 2
    print("All 5 independently re-verified before elevation. Maintenance unlocked for %d minutes." % TOKEN_MINUTES)
    return 0


def unlock_posix(root: Path, username: str) -> int:
    if os.name != "posix" or os.geteuid() != 0 or pwd is None:
        print("POSIX privileged verifier must run as root.", file=sys.stderr)
        return 2
    try:
    ...
    try:
        ...
        print("Invalid challenge: %s" % exc, file=sys.stderr)
        return 2
    if metadata.get("mode") != "locked" or int(time.time()) >= int(metadata.get("expiresAt", 0)):
        print("Locked challenge is invalid or expired.", file=sys.stderr)
        return 2
    for index, item in enumerate(metadata["problems"], 1):
        print("Privileged re-check %d/5: %s" % (index, item["id"]), flush=True)
        result = evaluate_problem(item["id"], int(item["seed"]), root / item["dir"] / "solution.py", user.pw_uid, user.pw_gid)
        if result.get("status") != "pass":
            print("Problem %d: FAIL (%s)" % (index, result.get("status")), file=sys.stderr)
            return 1
    token_path = _issue_token(metadata["id"], username, Path(user.pw_dir) / ".youtube-focus-lock")
    try:
...
def self_test() -> int:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        assert evaluate_problem(spec["id"], 99, path)["status"] == "wrong"
    print("challenge gate self-test PASS (%s)" % ("Windows" if os.name == "nt" else "POSIX"))
    return 0
...
def main() -> int:
    ...
    proof = sub.add_parser("create-proof")
    proof.add_argument("challenge_dir", type=Path)
    unlock = sub.add_parser("unlock")
    unlock.add_argument("challenge_dir", type=Path)
    unlock.add_argument("--user", required=True)
    unlock.add_argument("--platform", choices=("windows", "posix"), default="windows" if os.name == "nt" else "posix")
    valid = sub.add_parser("token-valid")
    valid.add_argument("--token", type=Path)
    ...
    if args.cmd == "start":
        print(write_challenge(args.mode)); return 0
    if args.cmd == "check":
        ...
        for i, item in enumerate(meta["problems"], 1):
            ...
            if result["status"] != "pass":
                print("Hint: %s" % hint_for_result(item["id"], result, 1)); return 1
        return 0
    if args.cmd == "create-proof":
        print(create_windows_proof(args.challenge_dir.resolve())); return 0
    if args.cmd == "unlock":
        return unlock_windows(args.challenge_dir.resolve(), args.user) if args.platform == "windows" else unlock_posix(args.challenge_dir.resolve(), args.user)
    if args.cmd == "token-valid":
        ok = token_valid(args.token or TOKEN_PATH); print("VALID" if ok else "INVALID"); return 0 if ok else 1
    if args.cmd == "_worker":
        if (args.uid is None) != (args.gid is None):
            print(json.dumps({"status": "runtime", "errorType": "JudgeConfigError", "message": "uid and gid must be supplied together"})); return 2
        payload = _worker_evaluate(args.problem_id, args.seed, args.solution.resolve(), args.uid, args.gid)
        print(json.dumps(payload, separators=(",", ":"))); return 0 if payload.get("status") in {"pass", "wrong", "runtime"} else 2
    return self_test()
